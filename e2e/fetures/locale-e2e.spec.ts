...
test.describe('Locales', () => {
  ...
  test('should check day time picker locale', async () => {
    ...
    expect(await po.weekDayNames().allTextContents()).toEqual([
      'א׳ב׳ג׳ד׳ה׳ו׳ש׳',
    ]);

    await po.daytimeInlineMenu().click();
    await po.localeSelect().selectOption('he');
    expect(await po.weekDayInline().allTextContents()).toEqual([
      'א׳ב׳ג׳ד׳ה׳ו׳ש׳',
    ]);
    ...
    expect(await po.calendarFirstMonthOfYear().first().textContent()).toEqual(
      'ינו',
    );
    ...
    await po.localeSelect().selectOption('he');
    expect(
      await po.calendarFirstMonthOfYearInline().first().textContent(),
    ).toEqual('ינו');
