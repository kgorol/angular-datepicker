...
describe('Service: Calendar', () => {
  ...
  it('should check the generateDaysMap method', inject(
    [DayCalendarService],
  ...
  it('should check the generateDaysMap method', inject(
    [DayCalendarService],
    (service: DayCalendarService) => {
      expect(service.generateDaysMap('su')).toEqual({
        su: 0,
        mo: 1,
        tu: 2,
        we: 3,
      ...
      expect(service.generateDaysMap('mo')).toEqual({
        mo: 0,
        tu: 1,
        we: 2,
        th: 3,
      ...
      expect(service.generateDaysMap('we')).toEqual({
        we: 0,
        th: 1,
        fr: 2,
        sa: 3,
  ...
  it('should check the generateMonthArray method', inject(
    [DayCalendarService],
    (service: DayCalendarService) => {
      ...
      monthWeeks = service.generateMonthArray(
        ...
        [
          dayjsRef('11-10-2016', 'DD-MM-YYYY'),
          dayjsRef('13-10-2016', 'DD-MM-YYYY'),
        ],
      );
  ...
  it('should check the generateWeekdays method', inject(
    [DayCalendarService],
    (service: DayCalendarService) => {
      expect(
  ...
  it('should check isDateDisabled method', inject(
    [DayCalendarService],
    (service: DayCalendarService) => {
      ...
      expect(
        service.isDateDisabled(dayjsRef('11-10-2016', 'DD-MM-YYYY'), config),
      ).toBe(true);
      expect(
        service.isDateDisabled(dayjsRef('12-10-2016', 'DD-MM-YYYY'), config),
      ).toBe(false);
      expect(
        service.isDateDisabled(dayjsRef('13-10-2016', 'DD-MM-YYYY'), config),
      ).toBe(false);
      expect(
        service.isDateDisabled(dayjsRef('14-10-2016', 'DD-MM-YYYY'), config),
      ).toBe(false);
      expect(
        service.isDateDisabled(dayjsRef('15-10-2016', 'DD-MM-YYYY'), config),
      ).toBe(true);
  ...
  it('should show/hide near month according to showNearMonthDays configuration', inject(
    ...
    (service: DayCalendarService) => {
      ...
      expect(
        service.generateMonthArray(
          config,
          dayjsRef('27-03-2017', 'DD-MM-YYYY'),
          [],
        ).length,
      ).toBe(6);
  ...
  it('should not effect the calendar when no full near weeks even if showNearMonthDays is false', inject(
    ...
    (service: DayCalendarService) => {
      ...
      expect(
        service.generateMonthArray(
          config,
          dayjsRef('27-04-2017', 'DD-MM-YYYY'),
          [],
        ).length,
      ).toBe(6);
    },
  ...
  it('should check getDayBtnText method', inject(
    [DayCalendarService],
    (service: DayCalendarService) => {
      ...
      expect(
        service.getDayBtnText({ dayBtnFormatter: () => 'bla' }, date),
      ).toEqual('bla');
      expect(
        service.getDayBtnText(
          { dayBtnFormat: 'DD', dayBtnFormatter: (m) => m.format('D') },
          date,
        ),
      ).toEqual('5');
    },
  ...
  it('should check getDayBtnCssClass method', inject(
    [DayCalendarService],
    (service: DayCalendarService) => {
      const date = dayjsRef('05-04-2017', 'DD-MM-YYYY');
      expect(service.getDayBtnCssClass({}, date)).toEqual('');
      expect(
        service.getDayBtnCssClass(
          { dayBtnCssClassCallback: () => 'class1 class2' },
          date,
        ),
