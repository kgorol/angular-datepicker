...
import {
  IDayTimeCalendarConfig,
  IDayTimeCalendarConfigInternal,
} from './day-time-calendar-config.model';
...
export class DayTimeCalendarService {
  ...
  updateDay(
    ...
    let updated = dayjsRef(
      day.format(DAY_FORMAT) + time.format(TIME_FORMAT),
      COMBINED_FORMAT,
    );
