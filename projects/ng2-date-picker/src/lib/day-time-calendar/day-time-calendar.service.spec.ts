...
describe('Service: DayTimeCalendarService', () => {
  ...
  it('should check the updateDay method', inject(
    [DayTimeCalendarService],
    (service: DayTimeCalendarService) => {
      ...
      expect(
        service.updateDay(daytime, day, {}).format(COMBINED_FORMAT),
      ).toEqual('1011020313:12:11');
    },
  ));
  ...
  it('should check the updateTime method when time is before min', inject(
    ...
    (service: DayTimeCalendarService) => {
      ...
      expect(
        service.updateDay(daytime, time, config).format(COMBINED_FORMAT),
      ).toEqual(daytime.add(10, 'm').format(COMBINED_FORMAT));

      expect(
        service.updateDay(daytime, time, {}).format(COMBINED_FORMAT),
  ...
  it('should check the updateTime method', inject(
    [DayTimeCalendarService],
    (service: DayTimeCalendarService) => {
      const daytime = dayjsRef('2011091313:12:11', COMBINED_FORMAT);
      const time = dayjsRef('03:11:10', TIME_FORMAT);
      expect(service.updateTime(daytime, time).format(COMBINED_FORMAT)).toEqual(
        '2011091303:11:10',
      );
    },
  ));
