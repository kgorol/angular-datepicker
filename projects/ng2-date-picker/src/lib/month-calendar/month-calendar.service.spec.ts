...
describe('Service: MonthCalendarService', () => {
  ...
  it('should check the isDateDisabled method', inject(
    [MonthCalendarService],
    (service: MonthCalendarService) => {
      const month: IMonth = {
  ...
  it('should check getDayBtnText method', inject(
    ...
    (service: MonthCalendarService) => {
      ...
      expect(service.getMonthBtnText({ monthBtnFormat: 'M' }, date)).toEqual(
        '4',
      );
      expect(service.getMonthBtnText({ monthBtnFormat: 'MM' }, date)).toEqual(
        '04',
      );
      expect(
        service.getMonthBtnText({ monthBtnFormatter: () => 'bla' }, date),
      ).toEqual('bla');
      expect(
        service.getMonthBtnText(
          { monthBtnFormat: 'MM', monthBtnFormatter: (m) => m.format('M') },
          date,
        ),
      ).toEqual('4');
    },
  ...
  it('should check getMonthBtnCssClass method', inject(
    [MonthCalendarService],
    (service: MonthCalendarService) => {
      const date = dayjsRef('05-04-2017', 'DD-MM-YYYY');
      expect(service.getMonthBtnCssClass({}, date)).toEqual('');
      expect(
        service.getMonthBtnCssClass(
          { monthBtnCssClassCallback: () => 'class1 class2' },
          date,
        ),
