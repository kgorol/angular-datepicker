  ...
  implements OnInit, OnChanges, ControlValueAccessor, Validator
{
  ...
  set currentDateView(current: Dayjs) {
    ...
    this.navLabel = this.monthCalendarService.getHeaderLabel(
      this.componentConfig,
      this.currentDateView,
    );
    this.showLeftNav = this.monthCalendarService.shouldShowLeft(
    ...
    this.showSecondaryLeftNav =
      this.componentConfig.showMultipleYearsNavigation && this.showLeftNav;
    this.showSecondaryRightNav =
      this.componentConfig.showMultipleYearsNavigation && this.showRightNav;
  }
  ...
  onLeftSecondaryNavClick(): void {
    ...
    const isOutsideRange =
      this.componentConfig.min &&
      this.currentDateView.year() - this.componentConfig.min.year() <
        navigateBy;
  ...
  onRightSecondaryNavClick(): void {
    ...
    const isOutsideRange =
      this.componentConfig.max &&
      this.componentConfig.max.year() - this.currentDateView.year() <
        navigateBy;
  ...
  getMonthBtnCssClass(month: IMonth): { [klass: string]: boolean } {
    ...
    const customCssClass: string =
      this.monthCalendarService.getMonthBtnCssClass(
        this.componentConfig,
        month.date,
      );
  ...
  handleConfigChange(config: SimpleChange): void {
    if (config) {
      const prevConf: IMonthCalendarConfigInternal =
        this.monthCalendarService.getConfig(config.previousValue);
      const currentConf: IMonthCalendarConfigInternal =
        this.monthCalendarService.getConfig(config.currentValue);
