...
describe('Service: ObUtilsService', () => {
  ...
  it('should check isDateValid method', inject(
    [UtilsService],
  ...
  it('should check updateSelected method for day', inject(
    [UtilsService],
    (service: UtilsService) => {
      const date1: IDate = {
      ...
      expect(
        service.updateSelected(true, [date1.date], date2, 'day').length,
      ).toEqual(2);

      date1.selected = true;
      expect(
        service.updateSelected(true, [date1.date], date1, 'day').length,
      ).toEqual(0);
    },
  ));

  it('should check updateSelected method for month', inject(
    [UtilsService],
    (service: UtilsService) => {
      const date1: IDate = {
        date: dayjsRef('21-04-2017', 'DD-MM-YYYY'),
      ...
      expect(
        service.updateSelected(true, [date1.date], date3, 'month').length,
      ).toEqual(2);

      date1.selected = true;
      expect(
        service.updateSelected(true, [date1.date], date1, 'month').length,
      ).toEqual(0);
    },
  ));

  it('should check if date is in range', inject(
    [UtilsService],
    (service: UtilsService) => {
      expect(
        service.isDateInRange(
          dayjsRef(),
          dayjsRef().subtract(1, 'd'),
          dayjsRef().add(1, 'd'),
        ),
      ).toBeTruthy();
      expect(
        service.isDateInRange(dayjsRef(), dayjsRef().subtract(1, 'd'), null),
      ).toBeTruthy();
      expect(
        service.isDateInRange(dayjsRef(), null, dayjsRef().add(1, 'd')),
      ).toBeTruthy();
      expect(
        service.isDateInRange(
          dayjsRef().subtract(2, 'd'),
          dayjsRef().subtract(1, 'd'),
          dayjsRef().add(1, 'd'),
        ),
      ).toBeFalsy();
      expect(
        service.isDateInRange(dayjsRef(), dayjsRef().add(3, 'd'), null),
      ).toBeFalsy();
      expect(
        service.isDateInRange(dayjsRef(), null, dayjsRef().subtract(3, 'd')),
      ).toBeFalsy();
      expect(service.isDateInRange(dayjsRef(), null, null)).toBeTruthy();
    },
  ));

  it('should convertPropsToDayjs method', inject(
    [UtilsService],
    (service: UtilsService) => {
  ...
  it('should test datesStringToStringArray', inject(
    [UtilsService],
    (service: UtilsService) => {
      expect(service.datesStringToStringArray('')).toEqual([]);
      expect(service.datesStringToStringArray('14-01-1984')).toEqual([
        '14-01-1984',
      ]);
      expect(service.datesStringToStringArray('14-01-1984|15-01-1984')).toEqual(
        ['14-01-1984', '15-01-1984'],
      );
      ...
      expect(service.datesStringToStringArray('14,01-1984| asdasd')).toEqual([
        '14,01-1984',
        'asdasd',
      ]);
    },
  ...
  it('check convertToString', inject(
    [UtilsService],
    (service: UtilsService) => {
      const format = 'MM/DD/YYYY';
      expect(service.convertToString(null, format)).toEqual('');
      expect(service.convertToString('', format)).toEqual('');
      expect(service.convertToString(dayjsRef(), format)).toEqual(
        dayjsRef().format(format),
      );
      expect(service.convertToString([dayjsRef()], format)).toEqual(
        dayjsRef().format(format),
      );
      expect(
        service.convertToString([dayjsRef(), dayjsRef().add(1, 'd')], format),
      ).toEqual(
        dayjsRef().format(format) +
          ' | ' +
          dayjsRef().add(1, 'd').format(format),
      );
      expect(
