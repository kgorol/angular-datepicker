...
export class TimeSelectService {
  ...
  getTimeFormat(config: ITimeSelectConfigInternal): string {
    return (
      (config.showTwentyFourHours
        ? config.hours24Format
        : config.hours12Format) +
  ...
  decrease(
    config: ITimeSelectConfigInternal,
    time: Dayjs,
    unit: TimeUnit,
  ): Dayjs {
    let amount: number = 1;
  ...
  shouldShowDecrease(
    ...
    return (
      ...
      (!config.minTime ||
        config.minTime.isSameOrBefore(this.utilsService.onlyTime(newTime)))
    );
  }
  ...
  shouldShowIncrease(
    ...
    return (
      ...
      (!config.maxTime ||
        config.maxTime.isSameOrAfter(this.utilsService.onlyTime(newTime)))
    );
  }
