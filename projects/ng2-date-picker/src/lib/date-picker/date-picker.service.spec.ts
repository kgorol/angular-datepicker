...
describe('Service: DatePicker', () => {
  ...
  it('should check getConfig method for dates format', inject(
    [DatePickerService],
    (service: DatePickerService) => {
      ...
      expect(
        (<Dayjs>config1.min).isSame(
          dayjsRef('2016-10-25', 'YYYY-MM-DD'),
          'day',
        ),
      ).toBe(true);
      expect(
        (<Dayjs>config1.max).isSame(
          dayjsRef('2017-10-25', 'YYYY-MM-DD'),
          'day',
        ),
      ).toBe(true);
      ...
      expect(
        (<Dayjs>config2.min).isSame(
          dayjsRef('2016-10-25', 'YYYY-MM-DD'),
          'day',
        ),
      ).toBe(true);
      expect(
        (<Dayjs>config2.max).isSame(
          dayjsRef('2017-10-25', 'YYYY-MM-DD'),
          'day',
        ),
      ).toBe(true);
      ...
      expect(service.getConfig({}, 'daytime').format).toEqual(
        'DD-MM-YYYY HH:mm:ss',
      );
